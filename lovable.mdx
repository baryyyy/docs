...
```
// כשלקוח לוחץ "שדרג ל-Premium"
async function handleUpgrade(userSubscriptionUid) {
  try {
    const changeRequest = await splitz.subscriptions.requestProductChange({
      subscription_uid: userSubscriptionUid,
      target_product_uid: 'prod_premium_123',
      return_url: 'https://myapp.com/upgrade-success'
    });

    // בדיקה אם צריך תשלום
    if (changeRequest.data.requires_payment) {
      // יש תשלום נוסף - נעביר לדף אישור
      console.log(`💳 צריך לשלם: ₪${changeRequest.data.prorated_amount}`);
      
      // הפנה את הלקוח לאישור התשלום
      window.location.href = changeRequest.data.approval_url;
      
    } else {
      // אין תשלום נוסף (Downgrade או זיכוי)
      console.log('✅ אין תשלום נוסף - השינוי יתבצע');
      
      if (changeRequest.data.prorated_amount < 0) {
        console.log(`🎁 זיכוי: ₪${Math.abs(changeRequest.data.prorated_amount)}`);
      }
      
      // אפשר להעביר ישירות לעמוד הצלחה
      window.location.href = 'https://myapp.com/upgrade-success?status=success';
    }
