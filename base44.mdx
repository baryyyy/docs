---
title: "חיבור סליקה לאפליקציית base44"
---
...
<Info>
  **דרישות התהליך:**
  ...
  - חבילת Builder לגישה לפונקציות backend
...
סך הכל: 2 מפתחות (API Key, Payment link uid)
...
**שימו לב שחובה להגדיר כתובת לחזרה לאחר תשלום מוצלחה (success url) על מנת שהמערכת תדע לאן להפנות את הלקוח שלכם לאחר תשלום מוצלח - תוכלו לבצע זאת בהגדרות לינק לתשלום.**
...
<Danger>
  ...
  תהליך התשלום שלכם מאובטח. בbase44 ניתן לעשות זאת באמצעות פיצר שנקרא Backend Function.
</Danger>

<Steps>
  <Step title="שלב 1: יצירת לינק תשלום">
    ```javascript
    ...
    // return the url to the frontend using return res.json({...
    ```

    This code & all of the other code blocks will be used only in the backend functions in Base44, and not in the frontend.

    All of the interaction of Base44 with Splitz must be implemented using backend, and the results will be passed to the frontend.
  </Step>
  <Step title="אימות תשלום וגישה למערכת">
    ...
    **הפרמטר שייחזור בURL ששמו: subscription_uid  יעזור לכם לזהות את המנוי ולאמת את התשלום.**

    **לדוגמא: **[**https://example.com/subscriptionCallback?subscription_uid=xxx**](https://example.com/subscriptionCallback?subscription_uid=xxx)
